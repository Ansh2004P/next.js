...
export default class ResponseCache implements ResponseCacheBase {
  ...
  private async handleGet(
    ...
    try {
      ...
      // Check if we can use the existing cache entry
      const shouldUseExistingCache =
        previousIncrementalCacheEntry &&
        !context.isOnDemandRevalidate &&
        (!previousIncrementalCacheEntry.isStale || context.isPrefetch)

      // Resolve early with existing cache if valid
      if (
        ...
        // Return early if cache is still valid as we've already resolved the
        // cache entry.
      ...
      if (context.isOnDemandRevalidate || !previousIncrementalCacheEntry) {
        resolve(incrementalResponseCacheEntry)
      }
      ...
      if (previousIncrementalCacheEntry !== null) {
        console.error(err)
        return null
      }
